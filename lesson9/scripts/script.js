async function resolveWeatherInfo() {
    ...
    fetch(`https://geocode.maps.co/search?q=${cite}`, {

    }).then(response => response.json()).then(citeInfo => {

        let result = document.getElementById("result");

        fetch(`https://api.open-meteo.com/v1/forecast?latitude=${citeInfo[0].lat}&longitude=${citeInfo[0].lon}&current_weather=true`, {
            method: "GET"
